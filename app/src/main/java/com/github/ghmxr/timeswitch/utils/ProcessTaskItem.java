...
public class ProcessTaskItem {
    ...
    public void activateTaskItem(){
        ...
        if(item.trigger_type ==PublicConsts.TRIGGER_TYPE_LOOP_WEEK){
            ...
            switch (dayofweek){
                default:break;
                case Calendar.MONDAY:if(!item.week_repeat[PublicConsts.WEEK_MONDAY]) canTrigger=false; break;
                case Calendar.TUESDAY:if(!item.week_repeat[PublicConsts.WEEK_TUESDAY]) canTrigger=false; break;
                case Calendar.WEDNESDAY:if(!item.week_repeat[PublicConsts.WEEK_WEDNESDAY]) canTrigger=false; break;
                case Calendar.THURSDAY:if(!item.week_repeat[PublicConsts.WEEK_THURSDAY]) canTrigger=false; break;
                case Calendar.FRIDAY:if(!item.week_repeat[PublicConsts.WEEK_FRIDAY]) canTrigger=false; break;
                case Calendar.SATURDAY:if(!item.week_repeat[PublicConsts.WEEK_SATURDAY]) canTrigger=false; break;
                case Calendar.SUNDAY:if(!item.week_repeat[PublicConsts.WEEK_SUNDAY]) canTrigger=false; break;
            }
        }
            ...
            if(Integer.parseInt(switch_values[0])>=0){
                for(String s:switch_values){
                    ...
                    if(cursor.getCount()>0){
                        ...
                        if(TimeSwitchService.list!=null&&position>=0&&position<TimeSwitchService.list.size()){
                            if(TimeSwitchService.list.get(position).trigger_type==PublicConsts.TRIGGER_TYPE_SINGLE&&TimeSwitchService.list.get(position).time<=System.currentTimeMillis()) continue;
                           if(enableOrDisable==0) {
                               TimeSwitchService.list.get(position).isenabled=true;
                               TimeSwitchService.list.get(position).activateTrigger(context);
                           }
                           else {
                               TimeSwitchService.list.get(position).isenabled=false;
                               TimeSwitchService.list.get(position).cancelTrigger();
                        ...
                        database.execSQL("update "+SQLConsts.getCurrentTableName(context)
                                +" set "+SQLConsts.SQL_TASK_COLUMN_ENABLED +"="+(enableOrDisable==0?1:0)+
